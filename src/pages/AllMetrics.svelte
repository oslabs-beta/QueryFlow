<script lang="ts">
	...
	function handleMouseOver(d: any) {
		showTooltip = true;
		tooltipContent = `Query: ${d.queryName} - Time: ${d.averageTime}ms`;
	...
	$: if (window.innerWidth > 1200) {
		...
		yScale.range([0, height / 1.5]).padding(barPadding * 1.5);
	} else {
		yScale.range([0, height]).padding(barPadding);
	}

	$: innerHeight = height - margin.top - margin.bottom;
	$: barPadding = data.length < 5 ? 0.5 : 0.1;
...
<div class="flex items-center justify-center mb-8 text-center pt-2">
	...
	<button class="btn btn-primary text-base ml-4" on:click={() => (formModal = true)}
		>Get Redis Metrics</button
	>
...
<div class="h-full w-full flex space-start" style="aspect-ratio: 16/9;">
	{#if width && height}
		<svg
			...
			class="h-full w-full all-metrics"
		>
			<g transform={`translate(0,${innerHeight})`}>
				<text
					transform={`translate(${width / 2},${margin.bottom + 27})`}
			...
			<g transform={`translate(${margin.left},${margin.top})`}>
				...
				{#each data as d}
					<text
						...
						x="-3"
						dy=".32em"
						y={yScale(d.queryName) + yScale.bandwidth() / 2}
					>
						{d.queryName}
					</text>
					<rect
