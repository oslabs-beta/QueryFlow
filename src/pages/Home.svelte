...
{#if loading}
	<div class="text-center mt-48"><Spinner color="green" size={8} /></div>
{:else}
	<div class="w-full flex flex-col items-center content-center justify-center mt-6">
		<div>
			...
			<label
				for="my-drawer"
				class="btn btn-primary drawer-button border-none hover:bg-secondary hover:text-white text-base"
				>Add a Query</label
			>
			<button
				on:click={() => navigate('/all-metrics')}
				class="ml-4 btn btn-primary drawer-button border-none hover:bg-secondary hover:text-white text-base"
				>All Metrics</button
			>
		</div>
		...
		<div
			class="mt-8 pb-10 w-screen grid sm:grid-cols-1 md:grid-cols-2 justify-center content-center"
		>
			<div class="h-screen scrollbar-hide overflow-y-auto column-width">
				...
				<select
					...
					on:change={() => {
						filterMetrics(filterMetricData, groupQueries);
					}}
				>
			...
			<div class="h-screen scrollbar-hide overflow-y-auto">
				...
				<select
					...
					on:change={() => {
						filterMetrics(filterMetricDataTwo, groupQueriesTwo);
					}}
				>
					<option value="all" selected hidden>Please choose...</option>
					{#each Array.from($uniqueNames) as value}
